<?php
	...
	class bf_woo_elem_product_shipping {
		public static function bf_wc_shipping( $thepostid, $customfield ) {
			?>
			...
			if ( wc_product_weight_enabled() ) {
				woocommerce_wp_text_input( array(
					'id'          => '_weight',
					'label'       => __( 'Weight', 'woocommerce' ) . ' (' . get_option( 'woocommerce_weight_unit' ) . ')<br>',
			...
			if ( wc_product_dimensions_enabled() ) {
				...
				<label
					for="product_length"><?php echo __( 'Dimensions', 'woocommerce' ) . ' (' . get_option( 'woocommerce_dimension_unit' ) . ')'; ?></label><br>
							<span class="wrap">
                ...
                ?>
								...
								<input id="product_length" placeholder="<?php _e( 'Length', 'woocommerce' ); ?>"
								       class="input-text wc_input_decimal dimensions_field" size="12" type="text"
								       name="_length" value="<?php echo $product_shipping_hidden_dimension_length ?>"/>
								<input placeholder="<?php _e( 'Width', 'woocommerce' ); ?>"
								       class="input-text wc_input_decimal dimensions_field" size="12" type="text"
								       name="_width" value="<?php echo $product_shipping_hidden_dimension_width ?>"/>
								<input placeholder="<?php _e( 'Height', 'woocommerce' ); ?>"
								       class="input-text wc_input_decimal last dimensions_field" size="12" type="text"
								       name="_height" value="<?php echo $product_shipping_hidden_dimension_height ?>"/>
							</span>
			...
			if ( isset( $customfield['product_shipping_hidden'] ) && in_array( 'hidden', $customfield['product_shipping_hidden'] ) ) {
				...
				woocommerce_wp_hidden_input( array( 'id'    => 'product_shipping_class',
				                                    'value' => $current_shipping_class
					) );
			} else {
				?>
				<label for="product_shipping_class"><?php _e( 'Shipping class', 'woocommerce' ); ?></label><p
					class="form-field dimensions_field"><?php wp_dropdown_categories( $args ); ?> <img class="help_tip"
