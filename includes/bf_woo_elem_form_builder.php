	...
	class bf_woo_elem_form_builder {
		...
		public function __construct() {
			add_filter( 'buddyforms_add_form_element_select_option', array(
				$this,
				'buddyforms_woocommerce_formbuilder_elements_select'
			), 1 );
			add_filter( 'buddyforms_form_element_add_field', array(
				$this,
				'buddyforms_woocommerce_create_new_form_builder_form_element'
			), 1, 5 );
		}
		...
		public function buddyforms_woocommerce_create_new_form_builder_form_element( $form_fields, $form_slug, $field_type, $field_id ) {
			...
			switch ( $field_type ) {
				...
				case 'woocommerce':
					...
					$element = new Element_Checkbox( 'Product Type Hidden', "buddyforms_options[form_fields][" . $field_id . "][product_type_hidden]", array( 'hidden' => '<b>' . __( 'Make the Product Type a Hidden Field', 'buddyforms' ) . '</b>' ), array(
